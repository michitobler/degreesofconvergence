...
```{r}
old.dat <- read.csv("FPKM_AllGenes.csv", row.names = 1)
keep <- rowSums(old.dat>2) >= (118*.66)
FPKM.filtered <- old.dat[keep,]
dim(FPKM.filtered)

FPKM.final <- FPKM.filtered

#Removing incomplete cases
FPKM.final[complete.cases(FPKM.final), ]

#Scaling
FPKM.final <- sqrt(FPKM.final)
```
...
LRT <- data.frame(matrix(nrow = 11390, ncol = 0))
for (i in 1:45) {
...
```{r}
# --- 1. WorldClim data ---
rc <- geodata::worldclim_global(var = "bio", res = 0.5, path = tempdir())
#saveRDS(rc, "rc.rds")
#rc <- readRDS("rc.rds")

# --- 2. Country boundaries ---
usa <- geodata::gadm("USA", level = 0, path = tempdir())
mex <- geodata::gadm("MEX", level = 0, path = tempdir())
dom <- geodata::gadm("DOM", level = 0, path = tempdir())

# safer merge
countries <- do.call(rbind, list(usa, mex, dom))

# --- 3. Crop to region ---
e <- ext(-100, -65, 12, 35)
range <- crop(rc, e)

# optional mask (only if you want to exclude ocean cells)
# range <- mask(range, countries)

# --- 4. Points ---
coords <- data.frame(x = env.var$long, y = env.var$lat)

# explicitly define CRS (WorldClim is WGS84)
points <- vect(coords, geom = c("x", "y"), crs = "EPSG:4326")

# ensure alignment (safe even if already correct)
points <- project(points, crs(range))

# --- 5. Extract ---
values <- terra::extract(range, points)[, -1, drop = FALSE]
...
write.csv(final.results3, "final.res.csv")
```

# 7. Phylogenetic dyadic model with reduced dimensionality

```{r}
df <- final.results3

df$conv_genes_sqrt <- scale(sqrt(df$conv.genes))
df$env_dist_s      <- scale(df$env.dist)
df$phylo_dist_s    <- scale(df$phylodist)

df$spec1 <- factor(df$spec1)
df$spec2 <- factor(df$spec2)


species <- unique(c(df$spec1, df$spec2))
phy_sub <- drop.tip(tree, setdiff(tree$tip.label, species))
plot(phy_sub)

#Covariance matrix
A <- vcv(phy_sub, corr = TRUE)

cor(df$env_dist_s, df$phylo_dist_s)
cor.test(df$env_dist_s, df$phylo_dist_s)

#Match factor levels
df$spec1 <- factor(df$spec1, levels = rownames(A))
df$spec2 <- factor(df$spec2, levels = rownames(A))


# Environmental variables (species-level table)
pc.vars <- env.var[, 6:29]
row.names(pc.vars) <- env.var$lineage

env.pca <- prcomp(pc.vars, scale. = TRUE)

# Extract PCA scores per species
pca.scores <- as.data.frame(env.pca$x)
pca.scores$species <- rownames(pca.scores)

# Choose PCs (edit if needed)
pc.use <- pca.scores[, c("PC1", "PC2", "PC3")]   
rownames(pc.use) <- pca.scores$species

# Euclidean distance in PCA space
pc.env.dist <- dist(as.matrix(pc.use))

#Prep for dyadic models
pc.env.long <- dist2list(as.dist(t(pc.env.dist)))
colnames(pc.env.long) <- c("spec1", "spec2", "pc.dist")
pc.env.long$combination <- paste(pc.env.long$spec1, pc.env.long$spec2)
pc.env.long <- pc.env.long[c(3,4)]

#Merge with dyadic df
df <- merge(df,
            pc.env.long,
            by = "combination",
            all.x = TRUE
...
#Scale predictor
df$pc_env_dist_s <- scale(df$pc.dist)

brm_m3 <- brm(
  conv_genes_sqrt ~ pc_env_dist_s + phylo_dist_s +
    (1 | gr(spec1, cov = A)) +
    (1 | gr(spec2, cov = A)),
  data = df,
  data2 = list(A = A),
...
loo1 <- loo(brm_m3, moment_match = FALSE, reloo = TRUE)
loo2 <- loo(brm_4, moment_match = FALSE, reloo = TRUE)
loo_compare(loo1, loo2)

conditional_effects(brm_4, effects = "phylo_dist_s:pc_env_dist_s")
test <- conditional_effects(brm_4, effects = "pc_env_dist_s:phylo_dist_s") 
...
#Plot
df_plot <- test[[1]]
ggplot(df_plot, aes(x = pc_env_dist_s,
                    y = estimate__,
                    color = factor(round(phylo_dist_s,0)),
                    fill = factor(phylo_dist_s))) +
  geom_line(linewidth = 1) +
  labs(
    x = "Environmental distance (scaled)",
    y = "Predicted response",
    color = "Phylogenetic distance",
...
```{r}
#Final p-value table
FDR2 <- as.data.frame(lapply(LRT[1:45], FDRcalc))
gene.names <- row.names(FPKM_11)
row.names(FDR2) <- gene.names

#Comparisons
comp <- read.csv("comparisons.csv")
#Change col names
colnames(FDR2) <- comp$name[match(colnames(FDR2), comp$X)]

#Annotations
annot <- read.csv("gene_annotations_min.csv", row.names = 1)

#Merge
summary.res <- merge(annot, FDR2, by=0, all.y=TRUE)

summary.res$counts <- rowSums(summary.res[, 5:49] < 0.01, na.rm = TRUE)

write.csv(summary.res, "eve_results_final.csv")
```
